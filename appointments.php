		         ...
		         <div class="panel-body minimal">
		            <div class="table-inbox-wrap ">
		                <table class="table table-inbox table-hover">
		                        ...
		                        {
		                        	$query = "SELECT * FROM ((appointments INNER JOIN users ON appointments.uid=users.id AND appointments.did='{$_SESSION['did']}')INNER JOIN doctor ON doctor.did='{$_SESSION['did']}') WHERE approval=1 AND appointments.date<'{$date1}' LIMIT 1";
		                        	$result = mysqli_query($conn,$query);
		                        
		                        }
		                        elseif(isset($_SESSION['uid']))
		                        {
		                        	$query = "SELECT * FROM appointments INNER JOIN users ON appointments.uid=users.id WHERE appointments.uid='{$_SESSION['uid']}' AND approval=1 AND appointments.date<'{$date1}' LIMIT 1";
		                        	$result = mysqli_query($conn,$query);
		                    ...
		                    <tbody>
		                            ...
		                            while ($row = mysqli_fetch_assoc($result)) {                            
		                                ...
		                                <tr class="">
		                                    ...
		                                    if(isset($_SESSION['did']))
		                                    {
			                                    $query1 = "SELECT * FROM users WHERE users.id='{$_SESSION['uid']}' LIMIT 1";
			                                    $result1 = mysqli_query($conn,$query1);
			                                    $row1 = mysqli_fetch_assoc($result1);
		                                	}
		                                	if(!isset($_SESSION['did']))
		                                	{
		                                		$query1 = "SELECT * FROM ((doctor INNER JOIN users ON doctor.uid=users.id) INNER JOIN appointments ON doctor.did=appointments.did) WHERE appointments.uid='{$_SESSION['uid']}' LIMIT 1";
			                                    $result1 = mysqli_query($conn,$query1);
			                                    $row1 = mysqli_fetch_assoc($result1);
		                                	}
		                                    ?>
                    ...
                    <tbody>
                            ...
                            while ($row = mysqli_fetch_assoc($result)) {                            
                                ...
                                <tr class="">
                                    ...
                                    <td class="view-message dont-show" ><?php echo $row['username']?></td>
                                    <?php 
                                    $query = "SELECT * FROM ((doctor INNER JOIN users ON doctor.uid=users.id) INNER JOIN appointments ON doctor.did=appointments.did) WHERE appointments.uid='{$_SESSION['uid']}' AND dapproved=1 LIMIT 1";
