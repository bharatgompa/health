         ...
         <div class="panel-body minimal">
            <div class="table-inbox-wrap ">
                <table class="table table-inbox table-hover">
                        <?php
                        $date1 =  date('Y-m-d');
                        //$query = "SELECT * FROM users,appointments INNER JOIN doctor ON appointments.did=doctor.did AND doctor.hid = '{$_SESSION['h_id']}'  WHERE approval=1 AND appointments.date<'{$date1}' LIMIT 1";
                        $query = "SELECT * FROM ((appointments INNER JOIN users ON appointments.uid=users.id) INNER JOIN doctor ON doctor.hid='{$_SESSION['h_id']}' AND appointments.did=doctor.did) WHERE approval=1 AND appointments.date<'{$date1}' LIMIT 1";
                    ...
                    <tbody>
                            ...
                            while ($row = mysqli_fetch_assoc($result)) {                            
                                ...
                                <tr class="">
                                    ...
                                    <?php 
                                    $query1 = "SELECT * FROM doctor INNER JOIN users ON doctor.uid=users.id AND doctor.hid='{$_SESSION['h_id']}' WHERE dapproved=1 LIMIT 1";
