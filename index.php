    ...
    if (isset($_POST['submit'])) {
        ...
        if ($error == "") {
                    ...
                    if (isset($row)) {
                        if($row['active']==1)
                        {
                           // $hashedPassword = ($row['id']).$_POST['password'];
                            ...
                            if ($hashedPassword == $row['password']) {
                                ...
                                if($row['user_type'] == 1)
                                {
                                    $query1 = "SELECT * FROM driver WHERE '".$row['id']."'=driver.uid";
                                    $result1 = mysqli_query($conn, $query1);
                                    $row1 = mysqli_fetch_array($result1);
                                    $_SESSION['drid'] = $row1['dr_id'];
                                }
                                if($row['user_type'] == 2)
                                {
                                    $query1 = "SELECT * FROM doctor WHERE '".$row['id']."'=doctor.uid";
                                    $result1 = mysqli_query($conn, $query1);
                                    $row1 = mysqli_fetch_array($result1);
